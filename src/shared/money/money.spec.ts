/**
 * CARRIED OVER VERBATIM from ../rwa-token-backend/test/money.test.ts — only the
 * import path changed. Per CLAUDE.md §9: ported hardened logic keeps its existing
 * tests, and this is the arithmetic that decides what each holder is actually
 * paid. It was missed when money.ts was ported; added when the distributions
 * module (its only caller) landed.
 */
import { describe, it, expect } from "vitest";
import { toPaise, fromPaise, sumPaise, allocatePayout } from "./money";
